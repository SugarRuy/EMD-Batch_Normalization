...
import cPickle as p
#My modified part
#fout = file('conv1_output_epoch-1_without_bn','w')
#fout.close()
...
convlayer2 = ConvLayer.ConvLayer(poollayer1.output, input_shape=(mini_batch_size, 32, 12, 12), filter_shape=(64, 32, 3, 3))
...
for epoch in range(10) : # to change epoch number, fix this number
	...
	#my modified 
	if epoch<=100000 : 
		np.savez('npzsave/withoutBN_%d'%epoch,now_input, conv1out(now_input), pool1out(now_input), conv2out(now_input), pool2out(now_input), conv3out(now_input))
