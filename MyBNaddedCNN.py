...
import cPickle as p
#My Modefied part 
#fout = file('conv1_output_epoch-1_with_bn','w')
...
for epoch in range(10) : # to modify epoch number, change this number 
	random_idx = np.arange(train_num)#random.permutation(train_num)
	...
	#edited part
	if epoch <= 1000000:
        	np.savez('npzsave/withBN_%d'%epoch,now_input, conv1out(now_input), pool1out(now_input), conv2out(now_input), pool2out(now_input), conv3out(now_input))                
